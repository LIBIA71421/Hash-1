...
public:
  ...
  void print(); 
	...
	T recursiveSearch(T dato, Par<T>* first);
};
...
template<class T>
T Lista7<T>:: recursiveSearch(T dato, Par<T>* first) {
		
		if (first == NULL) {
			return NULL;
		}
		else	if (first->getDato() == dato) {
			return first->getDato();
		}
		else {
			return recursiveSearch(dato,first->getSiguiente());
		}
